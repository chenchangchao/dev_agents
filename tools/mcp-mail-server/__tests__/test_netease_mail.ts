...
// 环境变量验证与配置
// --------------------------
const EMAIL_HOST = process.env.NETEASE_MAIL_HOST || "smtp.qq.com";
const EMAIL_PORT = parseInt(process.env.NETEASE_MAIL_PORT || "465");
const EMAIL_USER = process.env.NETEASE_MAIL_USER;
const EMAIL_PASS = process.env.NETEASE_MAIL_PASS;
const EMAIL_FROM = process.env.NETEASE_MAIL_FROM;
const EMAIL_GOTO = process.env.NETEASE_MAIL_GOTO;
const EMAIL_SUBJECT_TPL = process.env.NETEASE_MAIL_SUBJECT;
const EMAIL_CONTENT_TPL = process.env.NETEASE_MAIL_CONTENT;

// 严格验证所有必要环境变量
if (!EMAIL_USER || !EMAIL_PASS || !EMAIL_FROM || !EMAIL_GOTO || !EMAIL_SUBJECT_TPL || !EMAIL_CONTENT_TPL) {
  console.error("❌ 缺少必要的邮件环境变量，请检查.env文件");
  console.error("需要配置：NETEASE_MAIL_USER, NETEASE_MAIL_PASS, NETEASE_MAIL_FROM, NETEASE_MAIL_GOTO, NETEASE_MAIL_SUBJECT, NETEASE_MAIL_CONTENT");
  process.exit(1);
}
...
class EmailService {
  ...
  async sendVerificationEmail(to: string, code: string, expireMinutes: number = 5): Promise<boolean> {
    try {
      ...
      console.error(`❌ 验证码邮件发送失败，收件人：${to}`, err);
      return false;
    }
  }
  ...
  async sendEmail(to: string, subject: string, text: string, html?: string): Promise<boolean> {
    try {
      ...
      console.error(`❌ 邮件发送失败，收件人：${to}`, err);
      return false;
    }
  }
}
