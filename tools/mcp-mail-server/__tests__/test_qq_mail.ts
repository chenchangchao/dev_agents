...
// 环境变量验证与配置
// --------------------------
const EMAIL_HOST = process.env.QQ_MAIL_HOST || "smtp.qq.com";
const EMAIL_PORT = parseInt(process.env.QQ_MAIL_PORT || "465");
const EMAIL_USER = process.env.QQ_MAIL_USER;
const EMAIL_PASS = process.env.QQ_MAIL_PASS;
const EMAIL_FROM = process.env.QQ_MAIL_FROM;
const EMAIL_GOTO = process.env.QQ_MAIL_GOTO;
const EMAIL_SUBJECT_TPL = process.env.QQ_MAIL_SUBJECT;
const EMAIL_CONTENT_TPL = process.env.QQ_MAIL_CONTENT;

// 严格验证所有必要环境变量
if (!EMAIL_USER || !EMAIL_PASS || !EMAIL_FROM || !EMAIL_GOTO || !EMAIL_SUBJECT_TPL || !EMAIL_CONTENT_TPL) {
  console.error("❌ 缺少必要的邮件环境变量，请检查.env文件");
  console.error("需要配置：QQ_MAIL_USER, QQ_MAIL_PASS, QQ_MAIL_FROM, QQ_MAIL_GOTO, QQ_MAIL_SUBJECT, QQ_MAIL_CONTENT");
  process.exit(1);
}
